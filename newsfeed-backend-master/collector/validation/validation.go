...
func getSourceIdFromDataCollectorId(collectorId protocol.PanopticTask_DataCollectorId) string {
	...
	case protocol.PanopticTask_COLLECTOR_XUEQIU:
		return collector.XueqiuSourceId
	case protocol.PanopticTask_COLLECTOR_WUBLOCK123:
		return collector.Wublock123SourceId
	default:
		return ""
	}
}
...
func crossTaskMessageValidation(sharedContext *working_context.SharedContext) error {
	...
	if defaultUrl != "" && msg.Post.SubSource.AvatarUrl != defaultUrl {
		msg.Post.SubSource.AvatarUrl = defaultUrl
		Logger.LogV2.Warn("crawled message's avatar doesn't match the source's default avatar url: " + defaultUrl + " != " + msg.Post.SubSource.AvatarUrl)
	}
...
func validateMessageSubSourceIsSetCorrectly(msg *protocol.CrawlerMessage) error {
	if msg.Post.SubSource.AvatarUrl == "" && msg.Post.SubSource.SourceId != collector.WeixinSourceId {
...
func isPostWithEmptyContent(post *protocol.CrawlerMessage_CrawledPost) bool {
	return post.Content == "" && post.Title == "" && len(post.ImageUrls) == 0 && len(post.FilesUrls) == 0
}
...
func validateMessagePostIsSetCorrectly(msg *protocol.CrawlerMessage) error {
	...
	if (msg.Post.SharedFromCrawledPost == nil && isPostWithEmptyContent(msg.Post)) ||
		...
		if !strings.Contains(msg.Post.OriginUrl, "weixin") && msg.Post.SubSource.SourceId != collector.WeiboSourceId {
	...
	if msg.Post.ContentGeneratedAt.AsTime().After(time.Now()) {
		return errors.New("crawled post must not be created in the future. Old: " +
			fmt.Sprint(msg.Post.ContentGeneratedAt.String()) + ". New: " + fmt.Sprint(time.Now().String()))
	}
