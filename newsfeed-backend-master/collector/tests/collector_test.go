...
import (
	...
	. "github.com/rnr-capital/newsfeed-backend/collector/handler"
...
func TestJin10CrawlerWithTitle(t *testing.T) {
	...
	t.Run("[get message from dom element][flash] html with title and <br/>", func(t *testing.T) {
		...
		require.Equal(t, "https://flash.jin10.com/detail/20210926132904521100", msg.Post.OriginUrl)
		require.Equal(t, sourceId, msg.Post.SubSource.SourceId)
		require.Equal(t, "a.com", msg.Post.SubSource.OriginUrl)
...
func TestJin10CrawlerWithImage(t *testing.T) {
	...
	t.Run("[get message from dom element][keynews] html with image", func(t *testing.T) {
		...
		require.Equal(t, "https://flash.jin10.com/detail/20210925215015057100", msg.Post.OriginUrl)
		require.Equal(t, sourceId, msg.Post.SubSource.SourceId)
		require.Equal(t, "a.com", msg.Post.SubSource.OriginUrl)
...
func TestWisburg(t *testing.T) {
	job := protocol.PanopticJob{
		Tasks: []*protocol.PanopticTask{{
			TaskId:          "123",
			DataCollectorId: protocol.PanopticTask_COLLECTOR_WISBURG,
			TaskParams: &protocol.TaskParams{
				HeaderParams: []*protocol.KeyValuePair{},
				Cookies:      []*protocol.KeyValuePair{},
				SourceId:     "bb3c8ee2-c81e-43d9-8d98-7a6bb6ca0238",
				SubSources: []*protocol.PanopticSubSource{
					{
						Name:       "研究",
						Type:       protocol.PanopticSubSource_ARTICLE,
				...
				Params: &protocol.TaskParams_WisburgTaskParams{
					WisburgTaskParams: &protocol.WisburgParams{
						ChannelType: []protocol.WisburgParams_ChannelType{
							protocol.WisburgParams_CHANNEL_TYPE_RESEARCH,
							protocol.WisburgParams_CHANNEL_TYPE_VIEWPOINT,
						},
					},
				},
			},
			TaskMetadata: &protocol.TaskMetadata{
				ConfigName: "test_wisburg_config",
			},
		},
		},
	}
