...
func (o *Orchestrator) RunModule(ctx context.Context) error {
	...
	for msg := range messages {
		...
		go func(job *protocol.PanopticJob) {
			...
			if err != nil {
				Logger.LogV2.Error(fmt.Sprintf("fail to execute job: %s, error: %s", panopticJob.String(), err))
				return
			}

			err = o.PublishFinishedJob(res)
			if err != nil {
				Logger.LogV2.Error(fmt.Sprintf("fail to publish job into executed job channel, error: %s", err))
				return
			}
		}(&panopticJob)
...
func (o *Orchestrator) Shutdown() {
	o.executor.Shutdown()
	Logger.LogV2.Info("Module " + o.Config.Name + " gracefully shutdown")
}
