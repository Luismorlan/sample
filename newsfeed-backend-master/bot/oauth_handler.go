...
func AuthHandler(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !ok {
			Logger.LogV2.Error("got an oauth request without code")
		...
		if err != nil {
			Logger.LogV2.Error(fmt.Sprint("got invalid oauth code", err))
		...
		if !slackResp.Ok {
			Logger.LogV2.Error(fmt.Sprint("failed to fetch channel info from slack", slackResp))
			c.JSON(http.StatusBadRequest, gin.H{"error": "failed to fetch the channel info from slack, please contact the tech team"})
			return
		}

		Logger.LogV2.Info(fmt.Sprint("Bot added to a channel", slackResp))
		...
		if err != nil {
			Logger.LogV2.Error(fmt.Sprint("failed to save the channel to backend, contact tech please", err))
			return
		}
