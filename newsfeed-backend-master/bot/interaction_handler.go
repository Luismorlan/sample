...
func (a Action) GetColumnId() string {
	return a.ActionId
}

func (a Action) GetColumnName() string {
	return a.Value
}
...
func InteractionHandler(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if len(payload.Actions) == 0 {
			Logger.LogV2.Error(fmt.Sprintf("invalid payload without any action %v", payload))
		}
		...
		if res.RowsAffected == 0 {
			...
			Logger.LogV2.Error(fmt.Sprintf("get interaction from a non-existing channel: %s", payload.Container.ChannelID))
			return
		}
		...
		if action.IsSubsribe() {
			db.Transaction(func(tx *gorm.DB) error {
				db.Clauses(clause.OnConflict{DoNothing: true}).Create(&model.ChannelColumnSubscription{
					ChannelID: channel.Id,
					ColumnID:  action.GetColumnId(),
				})
				return nil
			})
			responseText = fmt.Sprintf("Successfully subscribed to %s", action.GetColumnName())
		} else {
			db.Where("column_id = ?", action.GetColumnId()).Delete(&model.ChannelColumnSubscription{})
			responseText = fmt.Sprintf("%s is unsubscribed", action.GetColumnName())
		}
