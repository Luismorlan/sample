...
func TestCreateUserAndValidate(t *testing.T, name string, userId string, db *gorm.DB, client *client.Client) (id string) {
	...
	require.Equal(t, 0, len(resp.CreateUser.PostsRead))
	require.Equal(t, 0, len(resp.CreateUser.SubscribedColumns))
...
func TestCreateColumnAndValidate(t *testing.T, userId string, name string, visibility model.Visibility, db *gorm.DB, client *client.Client) (id string, updatedAt string) {
	var resp struct {
		UpsertColumn struct {
			Id         string           `json:"id"`
			Name       string           `json:"name"`
	...
	query := fmt.Sprintf(`mutation {
		upsertColumn(input:{userId:"%s" name:"%s" visibility:%s}) {
			id
			name
			visibility
			updatedAt
	...
	createTime, _ := parseGQLTimeString(resp.UpsertColumn.CreatedAt)
	require.NotEmpty(t, resp.UpsertColumn.Id)
	require.Equal(t, name, resp.UpsertColumn.Name)
	require.Truef(t, time.Now().UnixNano() > createTime.UnixNano(), "time created wrong")

	return resp.UpsertColumn.Id, resp.UpsertColumn.UpdatedAt
}

// create feed with name, do sanity checks and returns its Id
// filterDataExpression in graphql input should have escapes \
//
func TestCreateFeedAndValidate(t *testing.T, userId string, name string, filterDataExpression string, subSourceIds []string, visibility model.Visibility, db *gorm.DB, client *client.Client) (id string, updatedAt string, columnId string) {
	var resp struct {
		UpsertFeed struct {
			Id                   string           `json:"id"`
			Name                 string           `json:"name"`
	...
	require.NotEmpty(t, resp.UpsertFeed.Columns)
	require.NotEmpty(t, resp.UpsertFeed.Columns[0].Id)

	var f model.Feed
	db.Preload("SubSources").First(&f, "id = ?", resp.UpsertFeed.Id)
	require.Equal(t, len(subSourceIds), len(f.SubSources))

	return resp.UpsertFeed.Id, resp.UpsertFeed.UpdatedAt, resp.UpsertFeed.Columns[0].Id
}

func TestUpdateFeed(t *testing.T, feed model.Feed, db *gorm.DB, client *client.Client, columnId string) string {
	var resp struct {
		UpsertFeed struct {
			Id                   string           `json:"id"`
			Name                 string           `json:"name"`
	...
	query := fmt.Sprintf(`mutation {
		upsertFeed(input:{feedId:"%s" columnId:"%s" userId:"%s" name:"%s" filterDataExpression:"%s" subSourceIds:%s visibility:%s}) {
		  id
		  name
		  createdAt
		  updatedAt
	  ...
	  `, feed.Id, columnId, feed.CreatorID, feed.Name, compactEscapedjson, subSourceIdsStr, feed.Visibility)
	...
	require.Truef(t, oldCreatedAt.Unix() == createTime.Unix(), "created time got changed, not expected")
...
func TestUserSubscribeColumnAndValidate(t *testing.T, userId string, columnId string, db *gorm.DB, client *client.Client) {
	var resp struct {
		Subscribe struct {
			Id string `json:"id"`
		} `json:"subscribe"`
	...
	client.MustPost(fmt.Sprintf(`mutation {
	  ...
	  `, userId, columnId), &resp)

	require.Equal(t, userId, resp.Subscribe.Id)
}

// create user to feed subscription, do sanity checks
func TestGetSubscriberCountAndValidate(t *testing.T, columnId string, count int, db *gorm.DB, client *client.Client) {
	var resp struct {
		AllVisibleColumns []struct {
			Id              string `json:"id"`
			SubscriberCount int    `json:"subscriberCount"`
		} `json:"allVisibleColumns"`
	}
	...
	for _, column := range resp.AllVisibleColumns {
		if column.Id != columnId {
			continue
		}
		require.Equal(t, count, column.SubscriberCount)
		break
	}
}

// TODO(boning): add this unit test
func TestDeleteColumnAndValidate(t *testing.T, userId string, columnId string, owner bool, db *gorm.DB, client *client.Client) {
	var resp struct {
		DeleteColumn struct {
			Id string `json:"id"`
		} `json:"deleteColumn"`
...
func TestQueryUserState(t *testing.T, userId string, columnIds []string, client *client.Client) {
	var res struct {
		UserState struct {
			User struct {
				Id                string `json:"id"`
	...
	Ids := []string{}
	for _, column := range res.UserState.User.SubscribedColumns {
		Ids = append(Ids, column.Id)
	}

	require.True(t, StringSlicesContainSameElements(columnIds, Ids))
}
