...
// for API request
export function setColumnNotificationSetting(payload: {
  columnId: string
  enableMobileNotification: boolean
}) {
  return createAction('SET_COLUMN_NOTIFICATION_SETTING', payload)
}

// for local state update
export function setColumnNotificationSettingLocally(payload: {
  columnId: string
  enableMobileNotification: boolean
}) {
  return createAction('SET_COLUMN_NOTIFICATION_SETTING_LOCALLY', payload)
}
