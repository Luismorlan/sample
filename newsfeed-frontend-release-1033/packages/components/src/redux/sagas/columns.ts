...
// construct profile url based on profileId and originaUrl
        ...
        upsertFeed: {
          __args: {
            input: {
              ...
              // TODO https://rrcapital.atlassian.net/browse/TECH-23
              // once backend supports it, uncomment it
  ...
  try {
    ...
    yield put(
      actions.fetchColumnDataSuccess({
        ...
        mobileNotification: feed.mobileNotification,
      }),
    )
  } catch (err) {
    yield put(actions.fetchColumnDataFailure({ columnId: column.id }))
...
function constructSetColumnNotificationSettingRequest(
  feedId: string,
  userId: string,
...
function* setColumnNotificationSetting(
  action: ExtractActionFromActionCreator<
    typeof actions.setColumnNotificationSetting
  >,
) {
  const { columnId, enableMobileNotification } = action.payload
  const appToken = yield* select(selectors.appTokenSelector)
  const userId = yield* select(selectors.currentUserIdSelector)
  if (!userId) {
    yield put(actions.authFailure(Error('no user id found')))
  ...
  const feedId = columnId
  try {
    yield axios.post(
      WrapUrlWithToken(constants.GRAPHQL_ENDPOINT, appToken || ''),
      {
        query: constructSetColumnNotificationSettingRequest(
          feedId,
          userId,
    ...
    yield put(
      actions.setColumnNotificationSettingLocally({
        columnId,
        enableMobileNotification,
      }),
    )
...
export function* columnsSagas() {
  yield* all([
    ...
    yield* takeEvery(
      'SET_COLUMN_NOTIFICATION_SETTING',
      setColumnNotificationSetting,
    ),
  ])
}
